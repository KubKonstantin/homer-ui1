...
export class TransactionServiceProcessor {
  public pipeDataTransaction(transactionData: any, _ipaliases: any) {
    ...
    if (!transactionData.data?.callid) {
      transactionData.data.callid = transactionData.data.messages
        .map((i) => i.callid)
        .sort()
    ...
    if (environment?.isHomerAPI) {
      fullCallData = fullCallData.filter(
        (i) => !(i.proto === 'rtcp' && i.typeItem === FlowItemType.SIP)
    ...
    return messages
      ...
      .map((item, pid) => {
        const transactionByCallId =
          transaction?.find((c) => c.callId === item.callId) || {};
        ...
        const eventName = i.raw
          ...
          : i.typeItem === FlowItemType.LOG
          ? FlowItemType.LOG
          : 'SIP/SDP';

        if (i.proto === 'log') {
          filters.payload['HEP-LOG']=true;
          i.typeItem='HEP-LOG';
        }
        ...
        if (
          !item.sdp &&
          i?.raw_source?.includes('Content-Type: application/sdp')
        ) {
          item.sdp = true;
        }
        ...
        const isRTP =
          i.typeItem === FlowItemType.RTP || i.typeItem === FlowItemType.RTCP;
        ...
        const codecString = `${name || '--'}/${!isNaN(rate) ? rate : '--'}/PT:${
          ...
          codecData,
          callid: i.callid,
          method_text: i.typeItem === 'HEP-LOG' ? 'HEP-LOG' : (i.sdp
